...
安装完成后默认每 6 小时自动检查更新。也可以随时把下面这句话发给 Hermes，
立即执行一次手动更新：
...
将 Plutus 安装到 `~/.plutus`，向 Hermes 注册 Plutus MCP，启动 Web、daemon
和自动更新 launchd 服务，并引导用户完成邮箱授权。
...
- 自动更新只能从安装仓库的 `origin/main` 快进，不得切换仓库、分支或覆盖本地修改。
...
5. 安装并启动 Web、daemon 和 updater 的 LaunchAgent。
...
安装器默认注册 `ai.plutus.updater`，每 6 小时执行一次
`~/.plutus/scripts/update.sh`。更新器会：

1. 确认当前分支是 `main` 且工作区干净。
2. 获取固定远端 `origin/main`。
3. 仅在可以快进时更新，不覆盖或删除本地修改。
4. 更新后重新运行安装器，刷新 MCP 和 LaunchAgent。
5. 更新成功、失败或因本地修改跳过时发送微信通知；没有更新时不通知。

日志位置：

```text
~/Library/Logs/plutus/update.log
~/Library/Logs/plutus/update.err.log
```

自动更新允许 `origin/main` 的新代码定期在本机执行。如果用户不接受这一信任边界，
可以禁用：

```bash
launchctl bootout "gui/$(id -u)/ai.plutus.updater"
rm ~/Library/LaunchAgents/ai.plutus.updater.plist
```

重新运行 `bash ~/.plutus/scripts/install_launchd.sh` 可以恢复自动更新。
...
- QQ 邮箱：填写 QQ 邮箱地址和 IMAP/SMTP 授权码。入口：右上角头像 → 设置 → 账号与安全 → 安全设置 → 开启服务 → 生成授权码。
- 163 邮箱：填写 163 邮箱地址和客户端授权码。入口：设置 → POP3/SMTP/IMAP → 新增授权密码。
- Gmail：填写 Gmail 地址和 Google 应用专用密码。入口：Google 账号 → 安全性 → 应用专用密码。

授权码不是邮箱登录密码。授权码只保存在 `~/.plutus/secrets/mail_auth.json`，不得粘贴到聊天中。
...
launchctl print "gui/$(id -u)/ai.plutus.updater"
...
- 三个 LaunchAgent 都已正确注册；Web 和 daemon 保持运行，updater 按周期启动。
- Web 控制台可以打开。
- MCP 检查通过。
- 数据库完整性检查返回 `ok`。
- 用户完成邮箱授权后，邮件自检通过。
