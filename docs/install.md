...
Plutus 当前仅支持 macOS。它通过 Gmail 读取招商银行交易邮件，将账本保存在本机，并使用 Hermes 完成 AI 分类和微信通知。
...
将 Plutus 安装到 `~/.plutus`，向 Hermes 注册 Plutus MCP，启动 Web 和 daemon 的 launchd 常驻服务，并引导用户完成 Gmail 授权。
...
- 不得要求用户在聊天中发送 Gmail 应用专用密码。用户必须在 Plutus 本地设置页中填写。
...
用户需要在设置页填写 Gmail 地址和 Google 应用专用密码。密码只保存在 `~/.plutus/secrets/`，不得粘贴到聊天中。
